...
export function ItemSelectorRow({ item, onToggle }: ItemSelectorRowProps) {
  return (
    <motion.div
      ...
      className={cn(
        "group flex cursor-pointer items-center gap-4 rounded-xl px-4 py-4 transition-colors duration-200",
        "hover:bg-zinc-50 dark:hover:bg-zinc-800/50",
        item.completed && "bg-zinc-50/50 dark:bg-zinc-800/30"
      )}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="relative inline-block">
          ...
          <WavyStrikethrough isVisible={item.completed} />
        </div>
